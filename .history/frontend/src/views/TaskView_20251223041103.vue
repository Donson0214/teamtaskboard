    ...
    <div class="px-4 sm:px-6 py-6 flex items-center justify-between">
      <div class="flex items-center gap-4">
        <button
          ...
          :class="isDark ? 'border-slate-800 bg-slate-900/80 hover:border-purple-500/60' : ' bg-white/70 hover:border-purple-300/70'"
        >
    ...
    <main class="flex-1 overflow-x-auto p-4 sm:p-6">
        ...
        <div v-for="column in orderedColumns" :key="column.id" class="w-full max-w-xs flex flex-col gap-2 transition hover:-translate-y-1 sm:w-80">
          ...
          >
            ...
            <div
              v-for="task in (filteredTasksByColumnId[column.id] || [])"
              :key="task.id"
...
const filteredTasksByColumnId = computed(() => {
  ...
  const { query, status, assignee, priority, dueDate } = taskFilter.value;
  ...
  (columns.value || []).forEach((column) => {
    ...
    if (!query && assignee === "all" && priority === "all" && !dueDate) {
...
const handleDrop = async (e, columnId) => {
  ...
  try {
    const ok = await boardStore.moveTask(draggedTaskId.value, columnId);
    if (!ok) {
      await loadBoardData();
