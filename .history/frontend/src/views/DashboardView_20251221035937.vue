            ...
            <div class="p-6 sm:p-7 space-y-6">
              ...
              <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                >
                  ...
                  <div v-if="stat.type === 'session'" class="relative w-12 h-12">
                    <svg viewBox="0 0 36 36" class="w-full h-full -rotate-90">
                      <circle
                        :class="isDark ? 'text-slate-800' : 'text-indigo-100'"
                        stroke="currentColor"
                        stroke-width="4"
                        fill="none"
                      ...
                      <circle
                        ...
                        stroke-width="4"
                        fill="none"
                        :stroke-dasharray="`${sessionProgress}, 100`"
                        d="M18 2a16 16 0 1 1 0 32 16 16 0 0 1 0-32"
                      />
                    </svg>
                    <div class="absolute inset-0 grid place-items-center text-[10px] font-semibold" :class="isDark ? 'text-white' : 'text-slate-800'">
                      {{ sessionProgress }}%
                    </div>
                  </div>
...
const sessionGoalHours = 8;
const sessionGoalSeconds = sessionGoalHours * 3600;

const sessionTimeLabel = computed(() => {
  const totalSeconds = Math.max(0, Math.floor(sessionSeconds.value));
...
const sessionProgress = computed(() => {
  if (!sessionGoalSeconds) return 0;
  const pct = (sessionSeconds.value / sessionGoalSeconds) * 100;
...
const headlineStats = computed(() => [
  {
    label: "Session time",
    value: sessionTimeLabel.value,
    helper: `of ${sessionGoalHours}h goal`,
    type: "session",
  },
  {
    label: "Open tasks",
...
const currentBoard = computed(
  () =>
    ...
    boardStore.boards?.find((board) => board.id === boardStore.boardId) ||
    boardStore.boards?.[0] ||
    null
);
