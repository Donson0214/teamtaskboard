  ...
  >
    ...
    <div class="flex min-h-0">
      ...
      <main class="flex-1 p-4 sm:p-6 overflow-auto min-h-0">
        <div class="max-w-7xl mx-auto h-full min-h-0">
          <div class="h-full rounded-2xl border overflow-hidden" :class="canvasSurface">
            <div class="grid h-full grid-rows-[minmax(0,1.35fr)_minmax(0,1fr)_minmax(0,0.8fr)]">
              <div
                class="grid grid-cols-1 md:grid-cols-[1.55fr,1fr] min-h-0 divide-y md:divide-y-0 md:divide-x"
                :class="divideClass"
              >
                <section class="p-4 sm:p-5 min-h-0 flex flex-col">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <p class="text-[11px] uppercase tracking-[0.2em]" :class="mutedText">Summary timeline</p>
                      <h2 class="text-lg sm:text-xl font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">Momentum</h2>
                    </div>
                    <div class="flex items-center gap-1 rounded-full border px-1 py-1" :class="dividerClass">
                      <button
                        class="px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase tracking-[0.18em] transition"
                    ...
                    <div v-if="momentumHasData" class="relative h-full">
                      <svg viewBox="0 0 100 40" preserveAspectRatio="none" class="w-full h-full pointer-events-none">
                        <defs>
                          <linearGradient id="summary-area" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" :stop-color="isDark ? 'rgba(99,102,241,0.4)' : 'rgba(99,102,241,0.28)'" />
                          ...
                          <linearGradient id="summary-area-soft" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" :stop-color="isDark ? 'rgba(14,165,233,0.25)' : 'rgba(56,189,248,0.18)'" />
                            <stop offset="100%" :stop-color="isDark ? 'rgba(15,23,42,0)' : 'rgba(255,255,255,0)'" />
                          </linearGradient>
                      ...
                      <div class="absolute inset-0 grid" :style="timelineGridStyle">
                        <div v-for="item in momentumSeries.items" :key="item.key || item.label" class="group relative">
                          <div
                            class="absolute left-1/2 top-2 -translate-x-1/2 rounded-md border px-2 py-1 text-[11px] font-medium opacity-0 transition group-hover:opacity-100"
                            :class="isDark ? 'bg-slate-900/90 text-white border-slate-700' : 'bg-white/90 text-slate-800 border-slate-200'"
                          >
                            <span class="font-semibold">{{ item.count }}</span>
                            <span class="ml-1 uppercase tracking-[0.16em]" :class="mutedText">{{ item.label }}</span>
                          </div>
                          <span
                      ...
                      <div class="absolute bottom-0 left-0 right-0 flex items-center justify-between text-[10px]" :class="mutedText">
                        <span
                          v-for="item in momentumSeries.items"
                ...
                <section class="p-4 sm:p-5 min-h-0 flex flex-col">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <p class="text-[11px] uppercase tracking-[0.2em]" :class="mutedText">Status breakdown</p>
                      <h3 class="text-base font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">Stages</h3>
                    </div>
                    <span class="text-[11px] font-semibold uppercase tracking-[0.16em]" :class="mutedText">
                      {{ statusMeta }}
                    </span>
                  </div>
                  <div class="mt-3 flex-1 min-h-0">
                    <div v-if="statusRings.length" class="grid grid-cols-[1fr,1fr] items-center gap-4 h-full">
                      <div class="flex items-center justify-center">
                        <div class="relative">
                          <svg viewBox="0 0 42 42" class="h-32 w-32">
                            <circle
                              cx="21"
                              cy="21"
                              r="15.915"
                          ...
                          <div class="absolute inset-0 grid place-items-center text-center">
                            <div>
                              <p class="text-xl font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">
                                {{ statusHighlight.percent }}%
                              </p>
                              <p class="text-[10px] uppercase tracking-[0.2em]" :class="mutedText">
                      ...
                      <div class="space-y-2">
                        <div v-for="segment in statusSegments.slice(0, 4)" :key="segment.label" class="flex items-center gap-2">
                          <span class="h-2.5 w-2.5 rounded-full bg-current" :class="segment.toneClass"></span>
                          <div class="flex-1 flex items-center justify-between text-[11px]">
                            <span class="truncate" :class="isDark ? 'text-slate-100' : 'text-slate-800'">{{ segment.label }}</span>
                            <span :class="mutedText">{{ segment.percentLabel }}</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div
                class="grid grid-cols-1 md:grid-cols-2 min-h-0 border-t divide-y md:divide-y-0 md:divide-x"
                :class="[dividerClass, divideClass]"
              >
                <section class="p-4 sm:p-5 min-h-0 flex flex-col">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <p class="text-[11px] uppercase tracking-[0.2em]" :class="mutedText">Activity</p>
                      <h3 class="text-base font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">Recent tasks</h3>
                    </div>
                    <span class="text-[11px] font-semibold uppercase tracking-[0.16em]" :class="mutedText">
                  ...
                  <div class="mt-3 flex-1 min-h-0">
                    <div v-if="activityFeed.length" class="space-y-2">
                      <div class="grid grid-cols-[minmax(0,1.4fr)_minmax(0,0.6fr)_minmax(0,0.6fr)] text-[10px] uppercase tracking-[0.18em] pb-2 border-b" :class="dividerClass">
                        <span :class="mutedText">Activity</span>
                        <span :class="mutedText">When</span>
                        <span :class="mutedText">Signal</span>
                      </div>
                      <div
                        v-for="item in activityFeed.slice(0, 4)"
                        :key="item.title + item.date"
                        class="grid grid-cols-[minmax(0,1.4fr)_minmax(0,0.6fr)_minmax(0,0.6fr)] items-center gap-3 border-b last:border-b-0 pb-2"
                        :class="dividerClass"
                      >
                        <div class="truncate text-sm font-medium" :class="isDark ? 'text-slate-100' : 'text-slate-900'">
                          {{ item.title }}
                        </div>
                        <div class="text-xs" :class="mutedText">{{ item.date }}</div>
                        <div class="h-1.5 rounded-full overflow-hidden" :class="isDark ? 'bg-slate-800' : 'bg-slate-100'">
                          <div
                            class="h-full rounded-full bg-gradient-to-r from-indigo-400 via-violet-400 to-sky-400"
                ...
                <section class="p-4 sm:p-5 min-h-0 flex flex-col">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <p class="text-[11px] uppercase tracking-[0.2em]" :class="mutedText">Column flow</p>
                      <h3 class="text-base font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">Bottlenecks</h3>
                    </div>
                    <span class="text-[11px] font-semibold uppercase tracking-[0.16em]" :class="mutedText">
                      {{ flowInsight }}
                    </span>
                  ...
                  <div class="mt-3 flex-1 min-h-0">
                    <div v-if="flowHasData" class="h-full flex items-end gap-2">
                      <div v-for="col in flowColumns" :key="col.key" class="flex-1 flex flex-col items-center gap-2">
                        ...
                        <span class="text-[10px] uppercase tracking-[0.2em] truncate" :class="mutedText">{{ col.label }}</span>
                      </div>
                    </div>
              ...
              >
                <section class="p-4 sm:p-5 min-h-0 flex flex-col">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <p class="text-[11px] uppercase tracking-[0.2em]" :class="mutedText">Metric rings</p>
                      <h3 class="text-base font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">Quick ratios</h3>
                    </div>
                  </div>
                  <div class="mt-3 grid grid-cols-2 gap-3">
                    <div v-for="ring in metricRings" :key="ring.id" class="flex items-center gap-3">
                      <div class="relative h-14 w-14">
                        ...
                        <div class="absolute inset-0 grid place-items-center text-[11px] font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">
                          {{ ring.value }}%
                        </div>
                      </div>
                      <div>
                        <p class="text-[10px] uppercase tracking-[0.2em]" :class="mutedText">{{ ring.label }}</p>
                ...
                <section class="p-4 sm:p-5 min-h-0 flex flex-col">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <p class="text-[11px] uppercase tracking-[0.2em]" :class="mutedText">Trend indicator</p>
                      <h3 class="text-base font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">Direction</h3>
                    </div>
                    <span class="text-[11px] font-semibold uppercase tracking-[0.16em]" :class="trendMeta.toneClass">
                      {{ trendMeta.label }}
                    </span>
                  </div>
                  <div class="mt-3 flex-1 min-h-0">
...
const flowColumns = computed(() => {
  ...
  return list.map((col) => ({
    ...col,
    height: Math.max(8, Math.round((col.count / max) * 100)),
