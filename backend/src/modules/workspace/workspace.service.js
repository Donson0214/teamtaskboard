...
module.exports.removeMember = async (workspaceId, memberId, userId) => {
  ...
  // Reload remaining members after deletion to ensure the leaver is not notified
  const remainingMembers = await prisma.workspaceMember.findMany({
    where: { workspaceId },
    select: { userId: true },
  });

  // Notify remaining members (including owner if still distinct) except the leaving user and the actor
  ...
  for (const recipientId of recipients) {
    if (recipientId === memberToRemove.userId) continue; // hard-stop: never notify leaver
    try {
      await notificationService.notifyUser(
        recipientId,
        message,
