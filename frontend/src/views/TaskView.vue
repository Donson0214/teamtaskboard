...
<div> 
  ...
  <div class="min-h-screen flex flex-col" :class="isDark ? 'bg-slate-950 text-slate-100' : 'bg-white text-slate-900'">
    ...
    >
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3 min-w-[260px]">
          <div
            class="w-10 h-10 rounded-2xl grid place-items-center border"
            :class="isDark ? 'border-slate-700 bg-slate-900' : 'border-slate-200 bg-slate-50'"
          >
            <i class="fas fa-table text-base text-slate-500"></i>
          </div>
          <div>
            <p class="text-[10px] uppercase tracking-[0.5em]" :class="isDark ? 'text-slate-400' : 'text-slate-500'">Board</p>
            <h2 class="text-xl font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">{{ currentBoard?.name || 'Board' }}</h2>
          </div>
          <div class="flex items-center gap-2">
                  <button
                    @click="goBack"
                    class="px-4 py-2 rounded-xl border text-sm font-semibold transition"
                    :class="isDark ? 'border-slate-800 text-slate-200 hover:border-purple-500/60 bg-slate-900/60' : 'border-slate-200 text-slate-800 hover:border-slate-300 bg-white/60'"
                  >
        ...
        <div v-for="column in orderedColumns" :key="column.id" class="w-full max-w-xs flex flex-col gap-2 transition hover:-translate-y-1 sm:w-80">
          ...
          >
            ...
            <div class="mt-2 flex gap-2" v-else>
              ...
              >
                <i class="fas fa-comment-dots mr-1 text-[10px]"></i> Comment
              </button>
              <button
                v-if="canEditTasks"
                class="px-3 py-1 text-[11px] rounded-full border"
              ...
              <button
                ...
                @click.stop="forceViewTask(task, column)"
              >
                <i class="fas fa-eye mr-1 text-[10px]"></i> View
              </button>
            </div>
...
const loadBoardData = async () => {
  if (!workspaceId.value || !boardId.value) return;

  await boardStore.loadBoard(workspaceId.value, boardId.value);
  ...
  socket.emit("joinBoard", boardId.value);
}
...
onUnmounted(() => {
  if (boardId.value) socket.emit("leaveBoard", boardId.value);
...
const moveTaskLocally = (taskId, targetColumnId) => {
