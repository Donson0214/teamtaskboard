  ...
  <div class="workspace-view min-h-screen relative overflow-hidden" :class="isDark ? 'bg-slate-950' : 'bg-slate-50'">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-3">
            <div class="logo-badge">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" class="w-7 h-7 text-white">
    ...
    <div class="flex">
      ...
      <transition name="slide">
                <aside
          v-show="sidebarOpen"
          class="w-[280px] border-r min-h-screen p-4 transition"
          ...
          <div class="flex flex-col h-full gap-5">
            ...
            <div class="flex-1 border-t pt-4" :class="isDark ? 'border-slate-800' : 'border-slate-200'">
              ...
              <div class="space-y-2 max-h-72 overflow-y-auto custom-scroll pr-1">
                <div
                  class="w-full flex items-center justify-between rounded-xl border px-3 py-2 text-sm font-semibold"
                  :class="isDark ? 'border-slate-800 text-slate-200' : 'border-slate-200 text-slate-800'"
                >
                  <div class="flex items-center gap-3">
                  ...
                  <button
                    class="w-8 h-8 rounded-xl border grid place-items-center text-sm transition"
                    :class="isDark ? ' text-slate-200' : 'border-slate-200 text-slate-600 hover:border-slate-300'"
              ...
              <div class="flex items-center justify-between flex-wrap gap-3">
                ...
                <p class="text-xs font-semibold uppercase tracking-wide" :class="isDark ? 'text-slate-500' : 'text-slate-500'">Spaces</p>
                  <h2 class="text-2xl font-bold" :class="isDark ? 'text-white' : 'text-slate-900'">
                    {{ userFirstName }}'s workspaces
                  </h2>
                  <p class="text-sm" :class="isDark ? 'text-slate-400' : 'text-slate-600'">
                    Browse and open {{ userFirstName }}'s spaces.
                  </p>
                </div>
              ...
              <div class="rounded-2xl border" :class="isDark ? 'bg-slate-900/70 border-slate-800' : 'bg-white border-slate-200 shadow-sm'">
                <div class="flex items-center justify-between gap-3 p-4">
                  <div class="flex items-center gap-2">
                    <i class="far fa-folder-open text-slate-500"></i>
                    <span class="text-sm font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">Workspaces</span>
                  </div>
                    ...
                    >
                      <div class="relative h-44">
                        ...
                        <button
                          ...
                          @click.stop="openWorkspaceMenu(workspace.id)"
...
const leaveWorkspaceTarget = ref(null);
...
const workspaceBeingEdited = ref(null);
...
const getRole = (workspaceId) => {
  return workspaceStore.getUserRole(workspaceId, user.value?.email);
};
...
const ownedWorkspaces = computed(() =>
  (filteredWorkspaces.value || []).filter((workspace) => getRole(workspace.id) === "owner")
);

const userEmailLower = computed(() => userEmail.value?.toLowerCase() || "");

const sharedWorkspaces = computed(() =>
  (filteredWorkspaces.value || []).filter((workspace) => {
    const role = getRole(workspace.id);
    const isMember = (workspace.members || []).some(
      (member) => member.email?.toLowerCase?.() === userEmailLower.value
    );

    return role !== "owner" && isMember;
  })
);

const workspacesToShow = computed(() => ownedWorkspaces.value);
...
const totalMembers = computed(() =>
  (filteredWorkspaces.value || []).reduce((sum, workspace) => {
...
const workspaceStatus = (workspace) => {
  const role = getRole(workspace.id);
  if (role === "owner") {
    return { label: "Owner", chip: "bg-white/80 text-blue-700 border border-blue-100 dark:bg-slate-900/70 dark:text-blue-200" };
  }
  if (role === "guest") {
    return { label: "guest", chip: "bg-white/80 text-slate-700 border border-slate-200 dark:bg-slate-900/70 dark:text-slate-200" };
  }
  return { label: "Member", chip: "bg-white/80 text-amber-700 border border-amber-100 dark:bg-slate-900/70 dark:text-amber-200" };
};
...
const handleNavClick = (item) => {
  ...
  if (item.id === "owned") activeFilter.value = "owned";
  else if (item.id === "shared") activeFilter.value = "shared";
...
const confirmLeaveWorkspace = async () => {
  ...
  try {
    await workspaceStore.leaveWorkspace(leaveWorkspaceTarget.value.id);
...
const handleSaveWorkspace = async ({ id, name }) => {
  if (id && name) {
    await workspaceStore.renameWorkspace(id, name);
  }
  closeEditWorkspace();
};
