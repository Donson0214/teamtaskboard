<template>
  <div class="workspace-view min-h-screen relative overflow-hidden pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]" :class="isDark ? 'bg-slate-950' : 'bg-slate-50'">
    ...
    >
      <div class="px-4 sm:px-6 py-6 flex items-center justify-end">
    ...
    >
      ...
      <main class="flex-1 p-4 sm:p-6 overflow-auto min-h-0 relative">
        <div
          v-if="isLoading"
        ...
        <div class="max-w-7xl h-full flex flex-col gap-6">
          <section class="rounded-2xl border" :class="heroSurface">
            <div class="p-6 sm:p-7 space-y-6">
                ...
                <div class="w-full sm:w-auto rounded-xl border px-4 py-3 flex flex-col gap-2 min-w-[220px]" :class="surfaceCard">
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="relative min-w-[140px]">
                      ...
                      <AppIcon
                        icon="fa-chevron-down"
                        class="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-[10px]"
                        :class="mutedText"
                      />
                    </div>
                  </div>
              ...
              <div class="grid gap-4 lg:grid-cols-2 items-stretch">
                <div class="rounded-xl border p-4 flex flex-col gap-4 min-h-56 overflow-hidden" :class="surfaceCard">
                  <div class="flex items-start justify-between gap-3">
                    <div class="space-y-1">
                      <p class="text-xs uppercase tracking-[0.12em]" :class="mutedText">Task activity</p>
                      <p class="text-lg font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">Activity volume</p>
                    </div>
                  ...
                  <div class="h-24 flex gap-2">
                    ...
                    <div class="flex-1">
                      ...
                      <div v-else class="h-full grid place-items-center text-xs" :class="mutedText">No task activity in range.</div>
                    </div>
                  </div>
                  <div v-if="taskVolumeHasData" class="flex gap-2">
                    <div class="w-10"></div>
                    <div class="flex-1 grid gap-2" :class="rangeGridClass">
                      <span
                  ...
                  <p class="text-[11px]" :class="mutedText">{{ taskVolumeTotal }} activity events - {{ progressRangeLabel }}</p>
                </div>

                <div class="rounded-xl border p-4 flex flex-col gap-3 min-h-56 overflow-hidden" :class="surfaceCard">
                  <div class="flex items-start justify-between gap-3">
                    <div class="space-y-1">
                  ...
                  <div v-if="progressHasData" class="flex gap-2">
                    <div class="w-10"></div>
                    <div class="flex-1 grid gap-0" :class="rangeGridClass">
                      <span
                  ...
                  <p class="text-[11px]" :class="mutedText">Status activity - {{ progressRangeLabel }}</p>
                </div>
              </div>
              ...
              <div class="grid gap-4 lg:grid-cols-[1.4fr,1fr]">
                ...
                <div class="rounded-xl border p-4 flex flex-col gap-4 min-h-56 overflow-hidden" :class="surfaceCard">
                  <div class="flex items-start justify-between gap-3">
                    ...
                    <div class="text-right">
                      <p class="text-sm font-semibold" :class="isDark ? 'text-white' : 'text-slate-900'">{{ activityToday }}</p>
                      <p class="text-[11px]" :class="mutedText">events today</p>
                    </div>
                  </div>
                  <div class="h-24 flex gap-2">
                    <div class="relative h-full w-10 text-[10px] leading-none" :class="mutedText">
                  ...
                  <div v-if="activityHasData" class="flex gap-2">
                    <div class="w-10"></div>
...
const CreateWorkspaceModal = defineAsyncComponent(() =>
  import("@/features/workspaces/modals/CreateWorkspaceModal.vue")
);
...
const scheduleRangeTick = () => {
  if (rangeTickTimer) clearTimeout(rangeTickTimer);
  const now = new Date();
  ...
  rangeTickTimer = setTimeout(() => {
    rangeTick.value = Date.now();
    scheduleRangeTick();
  }, delay);
};

const pruneStatusHistory = (history = {}) => {
...
const loadStatusHistory = () => {
  ...
  try {
    const key = statusHistoryKey.value;
    if (!key) {
      statusHistory.value = {};
      return;
    }
    const raw = localStorage.getItem(key);
    statusHistory.value = raw ? JSON.parse(raw) : {};
  } catch (err) {
    statusHistory.value = {};
  }
};

const persistStatusHistory = (history = {}) => {
  if (typeof window === "undefined") return;
  try {
    const key = statusHistoryKey.value;
    if (!key) return;
...
const backfillStatusHistory = (history = {}, todayKey) => {
  const keys = Object.keys(history).sort();
  if (!keys.length) return history;
  const lastKey = keys[keys.length - 1];
  if (lastKey === todayKey) return history;
  const lastDate = parseDateKey(lastKey);
  const todayDate = parseDateKey(todayKey);
  if (!lastDate || !todayDate) return history;

  let cursor = addDays(lastDate, 1);
  while (cursor < todayDate) {
    const key = formatDateKey(cursor);
    if (!history[key]) {
      history[key] = { ...history[lastKey] };
    }
...
const buildRangeBuckets = (range, now = new Date()) => {
...
const rangeBuckets = computed(() =>
  buildRangeBuckets(progressRange.value, new Date(rangeTick.value))
);

const getAuditActionKey = (entry = {}) =>
  String(entry.action || entry.message || "").toUpperCase();

const boardTaskIdSet = computed(() => {
  const set = new Set();
...
const getEntryTaskId = (entry = {}) =>
  entry.taskId || entry.details?.taskId || entry.details?.task?.id || entry.task?.id || null;

const isCommentAction = (key = "") => key.startsWith("COMMENT_");
const isTaskAction = (key = "") => key.includes("TASK");
const isMoveTaskAction = (key = "") => key.includes("MOVE") && key.includes("TASK");
const isCreateTaskAction = (key = "") => key.includes("CREATE") && key.includes("TASK");
const isCompleteTaskAction = (key = "") => key.includes("COMPLETE") && key.includes("TASK");

const taskActivityEntries = computed(() => {
  const entries = Array.isArray(boardActivity.value) ? boardActivity.value : [];
  return entries.filter((entry) => {
    const key = getAuditActionKey(entry);
    if (!isTaskAction(key) && !isCommentAction(key)) return false;
    if (!boardTaskIdSet.value.size) return true;
    const taskId = getEntryTaskId(entry);
    return taskId ? boardTaskIdSet.value.has(String(taskId)) : false;
  });
});

const taskActivityDates = computed(() => {
  const dates = [];
  const seenTaskDays = new Set();

  taskActivityEntries.value.forEach((entry, index) => {
    const date = parseDate(entry.createdAt);
    if (!date) return;
    const taskId = getEntryTaskId(entry) || `entry-${index}`;
    if (taskId) {
      seenTaskDays.add(`${taskId}-${formatDateKey(date)}`);
    }
  ...
  allTasks.value.forEach((task, index) => {
    ...
    if (taskId && seenTaskDays.has(`${taskId}-${formatDateKey(date)}`)) return;
    dates.push(startOfDay(date));
  });
...
const taskVolumeBuckets = computed(() =>
  bucketizeDates(taskActivityDates.value, rangeBuckets.value)
);
...
const columnStatusById = computed(() => {
  const map = new Map();
  (columns.value || []).forEach((col) => {
    const id = col?.id ?? col?._id;
    if (!id) return;
    const title = col.title || col.name || "";
    const status = isDoneColumn(title)
      ? "done"
      : isReviewColumn(title)
        ? "review"
        : "todo";
    map.set(String(id), status);
  });
  return map;
});

const resolveColumnId = (value) => {
  if (!value) return null;
  if (typeof value === "object") {
    return value.id || value._id || value.columnId || value.toColumnId || null;
  }
  return value;
};

const resolveMoveStatus = (details = {}) => {
  const columnId = [
    resolveColumnId(details.toColumnId),
    resolveColumnId(details.toColumn),
    resolveColumnId(details.to),
    resolveColumnId(details.destination),
  ].find(Boolean);

  if (columnId) {
    return columnStatusById.value.get(String(columnId)) || null;
  }

  const title = [
    details.toColumnTitle,
    details.toColumnName,
    details.toTitle,
    details.toName,
    typeof details.to === "string" ? details.to : null,
  ].find((value) => typeof value === "string" && value.trim());

  if (title) {
    if (isDoneColumn(title)) return "done";
    if (isReviewColumn(title)) return "review";
    return "todo";
  }
...
const taskStatusById = computed(() => {
  ...
  tasksWithStatus.value.forEach((task) => {
    const id = task?.id ?? task?._id;
    if (!id) return;
    map.set(String(id), getStatusKey(task));
  });
  return map;
});
...
const currentStatusCounts = computed(() => {
  ...
  tasksWithStatus.value.forEach((task) => {
    totals[getStatusKey(task)] += 1;
  });
  return totals;
});

const updateStatusHistory = () => {
  if (!statusHistoryKey.value) return;
  const todayKey = formatDateKey(new Date(rangeTick.value));
  const snapshot = {
    ...currentStatusCounts.value,
    updatedAt: new Date().toISOString(),
  };
  const base = { ...(statusHistory.value || {}) };
  const backfilled = backfillStatusHistory(base, todayKey);
  const next = pruneStatusHistory({
    ...backfilled,
    [todayKey]: snapshot,
  });
  statusHistory.value = next;
  persistStatusHistory(next);
};
...
const completionDates = computed(() => {
  if (taskActivityEntries.value.length) {
    const movedToDone = taskActivityEntries.value
      .filter((entry) => isMoveTaskAction(getAuditActionKey(entry)))
      .map((entry) => {
        const status = resolveMoveStatus(entry.details || {});
        if (status !== "done") return null;
        return parseDate(entry.createdAt);
      })
      .filter(Boolean)
      .map((date) => startOfDay(date));
    if (movedToDone.length) return movedToDone;
  }

  return tasksWithStatus.value
...
const statusSnapshots = computed(() => {
  const history = statusHistory.value || {};
...
const statusTrend = computed(() => {
  ...
  const snapshots = statusSnapshots.value;
  const fallback = currentStatusCounts.value;
  const today = startOfDay(new Date(rangeTick.value));
  let snapshotIndex = 0;
  ...
  buckets.forEach((bucket) => {
    while (
      ...
      snapshots[snapshotIndex].date < bucket.end
    ) {
    ...
    if (lastSnapshot) {
      bucket.todo = lastSnapshot.todo || 0;
      bucket.review = lastSnapshot.review || 0;
      bucket.done = lastSnapshot.done || 0;
      return;
    }

    if (today >= bucket.start && today < bucket.end) {
      bucket.todo = fallback.todo;
      bucket.review = fallback.review;
      bucket.done = fallback.done;
    }
  });
...
const statusTotals = computed(() => currentStatusCounts.value);

const statusMax = computed(() => {
  const peaks = (statusTrend.value || []).map((bucket) =>
...
const completedInRange = computed(() => statusTotals.value.done);
...
const activityDates = computed(() => {
  ...
  activity.forEach((entry, index) => {
    const date = parseDate(entry.createdAt || entry.updatedAt);
    if (!date) return;
    const taskId = entry.taskId || entry.details?.taskId || entry.details?.task?.id;
    if (taskId) {
      seenTaskDays.add(`${taskId}-${formatDateKey(date)}`);
    }
    dates.push(startOfDay(date));
...
const activityBuckets = computed(() =>
  buildRangeBuckets("week", new Date(rangeTick.value))
);
...
const activityToday = computed(() => {
  const buckets = activityTrend.value || [];
...
const statusHistoryKey = computed(() => {
  const workspaceId = selectedWorkspaceId.value;
  const boardId = selectedBoardId.value || boardStore.boardId;
  if (!workspaceId || !boardId) return "";
  return `taskflow-status-history:${workspaceId}:${boardId}`;
});
...
onMounted(async () => {
  try {
    ...
    console.error("Dashboard workspace load error:", err);
  }
...
watch(
  ...
  async (workspaceId) => {
    ...
    try {
      await withLoading(() => boardStore.loadBoards(workspaceId));
      ...
      selectedBoardId.value = currentMatch || storedMatch || fallback || "";
    } catch (err) {
...
watch(
  ...
  async (boardId) => {
    ...
    try {
      await withLoading(() =>
        boardStore.loadBoard(workspaceId, normalizedBoardId)
      );
      setStoredBoardId(workspaceId, normalizedBoardId);
    } catch (err) {
      console.error("Dashboard board load error:", err);
