<!-- src/features/boards/pages/BoardView.vue -->
<template>
  <div class="min-h-screen relative overflow-hidden pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]" :class="isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="relative" ref="profileRef">
            <button
              ...
              class="flex items-center gap-2 px-2.5 py-1.5 min-h-[44px] rounded-xl border transition shadow-sm"
              :class="[
                isDark
            ...
            <transition name="fade-scale">
              <div
                ...
                class="absolute right-0 mt-2 w-[min(16rem,calc(100vw-2rem))] rounded-2xl border py-3 shadow-xl z-50"
    ...
    >
      ...
      <!-- MAIN -->
      <main class="p-4 sm:p-6 flex-1 min-w-0 relative">
        <div
          v-if="isLoading"
        ...
        <div class="max-w-6xl space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div
              ...
              :class="[
                isDark ? 'bg-slate-900/70 border-slate-800 hover:border-purple-500/60' : 'bg-white/90 border-slate-200 hover:border-purple-300/70',
                openBoardDropdown === board.id ? 'z-20' : ''
              ]"
            >
              <div
              ...
              >
                <AppIcon icon="fa-ellipsis-h" class="text-sm" :class="isDark ? 'text-slate-300' : 'text-slate-600'" />
              </button>
              ...
              >
                <ul class="py-2 text-sm">
                  ...
                  >
                    <AppIcon icon="fa-eye" class="text-xs" />
                    View Board
                  </li>
                  ...
                  <li
                    ...
                    class="px-4 py-2 cursor-pointer flex items-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"
                  >
                    <AppIcon icon="fa-edit" class="text-xs" />
                    Edit Board
                  </li>
              ...
              <div class="relative flex items-center gap-3">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-2 text-sm" :class="isDark ? 'text-slate-400' : 'text-slate-600'">
                    <AppIcon icon="fa-list-check" class="text-xs" />
        ...
        >
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-red-100 text-red-600 flex items-center justify-center">
              <AppIcon icon="fa-exclamation-triangle" />
            </div>
            <div>
...
<script setup>
import { ref, shallowRef, onMounted, computed, onBeforeUnmount, watch, defineAsyncComponent } from "vue";
...
const CreateBoardModal = defineAsyncComponent(() =>
  import("@/features/boards/modals/CreateBoardModal.vue")
);
const EditWorkspaceModal = defineAsyncComponent(() =>
  import("@/features/workspaces/modals/EditWorkspaceModal.vue")
);
const InviteMembersModal = defineAsyncComponent(() =>
  import("@/features/workspaces/modals/InviteMembersModal.vue")
);
const ManageMembersModal = defineAsyncComponent(() =>
  import("@/features/workspaces/modals/ManageMembersModal.vue")
);
const EditBoardModal = defineAsyncComponent(() =>
  import("@/features/boards/modals/EditBoardModal.vue")
);
...
watch(
  () => boards.value.map((board) => board.id),
  () => {
    loadBoardTaskCounts(boards.value);
  }
);
...
const countTasksFromBoard = (boardData) => {
  const columns = boardData?.columns || [];
  return columns.reduce((sum, column) => sum + (column.tasks?.length || 0), 0);
};

const fetchBoardTaskCount = async (boardId) => {
  if (!workspaceId.value || !boardId) return null;
  try {
    const res = await api.get(`/workspaces/${workspaceId.value}/board/${boardId}`);
    const boardData = res.data?.data || res.data;
    return countTasksFromBoard(boardData);
  } catch (err) {
    return null;
  }
};
...
const loadBoardTaskCounts = async (list = boards.value) => {
  const currentRequest = ++taskCountRequestId;
  if (!list.length) {
    boardTaskCounts.value = {};
    return;
  }
  ...
  const results = await Promise.allSettled(
    list.map(async (board) => {
      const id = board?.id;
      if (!id) return null;
      const count = await fetchBoardTaskCount(id);
      return { id, count };
    })
  );

  if (!isActive || currentRequest !== taskCountRequestId) return;

  const nextCounts = { ...boardTaskCounts.value };
  ...
  boardTaskCounts.value = nextCounts;
};

const getBoardTaskCount = (board) => {
  const id = board?.id;
  if (id && typeof boardTaskCounts.value[id] === "number") {
    return boardTaskCounts.value[id];
  }
  return typeof board?.tasksCount === "number" ? board.tasksCount : 0;
};
...
const loadWorkspaceData = async (id) => {
  ...
  return withLoading(async () => {
    ...
    await loadBoardTaskCounts(loadedBoards || []);
  });
};
...
const handleOutsideClick = (e) => {
  ...
  if (openBoardDropdown.value) {
    const target = e.target;
    const clickedMenu = target?.closest?.("[data-board-menu]");
    const clickedTrigger = target?.closest?.("[data-board-menu-trigger]");
