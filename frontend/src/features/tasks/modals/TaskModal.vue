      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="space-y-5">
            ...
            <div>
              <label class="text-sm font-semibold flex items-center gap-2">
                <AppIcon icon="fa-tags" class="text-xs" />
                Labels
              </label>
              <div class="flex flex-wrap gap-2 mt-2">
                <button
                ...
                <p v-if="!labels?.length" class="text-xs" :class="isDark ? 'text-slate-400' : 'text-slate-500'">
                  No labels yet.
                </p>
              </div>
            </div>
              ...
              <div class="mt-2">
                ...
                >
                  ...
                  <input
                    ...
                    multiple
                    class="hidden"
                    accept=".jpg,.jpeg,.png,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation,text/plain,text/csv,image/jpeg,image/png"
                    @change="onFileUpload"
                  />
                </label>
                <p class="text-[11px] mt-1" :class="isDark ? 'text-slate-400' : 'text-slate-500'">
                  JPG/PNG, PDF, Word, Excel, PowerPoint, TXT/CSV (max 10MB).
                </p>
              </div>
...
import { PRIORITY_OPTIONS, findPriorityOption } from "@/shared/constants/priorities";
...
const labelStyle = (label) => {
  const active = selectedLabels.value.includes(label.name);
  return {
    backgroundColor: active ? label.color || "#6366f1" : "transparent",
    borderColor: label.color || "#6366f1",
    color: active ? "#fff" : isDark.value ? "#e2e8f0" : "#334155",
  };
};
